...
// -----------------------------------------------------------------------------
// E1 (model-pool): Candidate A instruction supply for the BERT program pool.
//
// Same fetch/decode/hold logic as exp1_candidate_a_pool_top, with BERT-fixed
// SRAM caliber:
//   config : sram_8192x5_wrapper (BERT config depth 8192)
//   payload: 5x sram_2048x34_tiled #(TILES=2) -> 4096 deep per slot bank
//            (BERT hottest slot needs 4096).  Uniform 2048x34 primitive with
//            per-tile CEN gating, so a payload read costs the same energy as
//            the E2-2/E2-3/E2-4/E2-5 BERT variants (lane-sweep fairness,
//            2026-09-16).  Native 4096x34 was used before; its per-read energy
//            is ~1.32x a gated 2048x34 tile and inflated E1's measured power.
//
...
module exp1_candidate_a_pool_top_bert (
    input  wire         clk,
    input  wire         rst_n,
    ...
    // config / payload SRAM address widths (BERT: 8192-deep config,
    // 4096-deep payload banks from 2x 2048x34 tiles)
    localparam integer CFG_AW = 13;
