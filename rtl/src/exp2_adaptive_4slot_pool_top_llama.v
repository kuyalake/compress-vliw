...
// the LLaMA program pool.
//
// Same fetch/decode/hold logic as exp2_adaptive_4slot_top, with LLaMA-fixed
// SRAM caliber (uniform gated-tile construction, lane-sweep fairness
// 2026-09-16):
//   config : sram_8192x5_tiled #(TILES=4) -> 32768 deep (LLaMA config)
//   lanes  : 4x sram_4096x34_tiled #(TILES=4) -> 16384 deep per lane
//            (LLaMA longest lane 8276).  Every payload read activates exactly
//            one 4096x34 tile, the same per-read energy as the E1/E2-2/E2-3/
//            E2-5 LLaMA variants.
//
...
module exp2_adaptive_4slot_pool_top_llama (
    input  wire         clk,
    input  wire         rst_n,
    ...
    // 4x 8192x5 tiles, 16384-deep payload lanes from 4x 4096x34 tiles)
    ...
    // payload lanes: 4x (4x 4096x34 gated tiles) -> 16384 deep each
    ...
    sram_4096x34_tiled #(.TILES(4)) u_lane3 (
