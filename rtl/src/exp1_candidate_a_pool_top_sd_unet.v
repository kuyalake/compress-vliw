...
// -----------------------------------------------------------------------------
// E1 (model-pool): Candidate A instruction supply for the SD-UNet program pool.
//
// Same fetch/decode/hold logic as exp1_candidate_a_pool_top, with SD-UNet-fixed
...
//   payload: 5x sram_4096x34_tiled #(TILES=2) -> 8192 deep per slot bank
//            (SD-UNet hottest slot 5696).  Every payload read activates
//            exactly one 4096x34 tile, the same per-read energy as the
//            E2-2/E2-3/E2-4/E2-5 SD-UNet variants.
//
...
module exp1_candidate_a_pool_top_sd_unet (
    input  wire         clk,
    input  wire         rst_n,
    ...
    // 2x 8192x5 tiles, 8192-deep payload banks from 2x 4096x34 tiles)
    localparam integer CFG_AW = 14;
    localparam integer PAY_AW = 13;
