...
// -----------------------------------------------------------------------------
// E2: adaptive 4-slot (four shared lanes) instruction supply
// (rtl/plan.md section 5.3) -- the main proposal; timing/area critical.
//
// Storage: config 1x sram_8192x5 (5-bit mask per cycle, T entries incl. the
// in-band EOP code 11111) + payload 4x sram_2048x34 (shared lanes 0..3).
//
// Encoding (compiler): active slots in ascending slot order (LOAD=0..SFU=4);
// rank r-th active event -> lane (phase + r) mod 4; phase += popcount(mask).
//
// Pipeline (II=1, 3-cycle latency), config one stage ahead of payload:
//   stage A: config address = cfg_pc (registered) -> config macro;
//   stage B: config Q = mask(k). Combinational: popcount, per-lane enables
//            en(i) = ((i - phase) mod 4) < popcount  -> lane CENs/addresses
//            (critical path 1: config Q -> decode -> payload CEN/A setup).
//            Registered at end of B: mask_c, phase_c (piped), lane pointers
//            (+= en), phase (+= popcount);
//   stage C: payload Q -> 4:1 scatter -> field output registers (hold
//            semantics). Scatter selects are computed from mask_c/phase_c
//            (both registers, stable early in the cycle), so the stage-C
//            payload path is just macro Q -> mux -> field register.
//
...
module exp2_adaptive_4slot_top (
    input  wire         clk,
    input  wire         rst_n,
    ...
    // ------------------------------------------------------------------
    reg  [1:0]  phase_q;    // phase of the instruction currently in stage B
    reg  [10:0] ptr0, ptr1, ptr2, ptr3;
    reg         c_vld;
    reg         c_eop;
    reg  [4:0]  mask_c;
    reg  [1:0]  phase_c;    // phase piped to stage C (for the scatter)
    ...
    // ------------------------------------------------------------------
    // rank of slot s = prefix popcount of mask_c below bit s
    ...
    // source lane of slot s = (phase + rank(s)) mod 4
