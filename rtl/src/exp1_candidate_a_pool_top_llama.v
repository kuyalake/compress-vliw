...
// -----------------------------------------------------------------------------
// E1 (model-pool): Candidate A instruction supply for the LLaMA program pool.
//
// Same fetch/decode/hold logic as exp1_candidate_a_pool_top, with LLaMA-fixed
// SRAM caliber (uniform gated-tile construction, lane-sweep fairness
// 2026-09-16):
//   config : sram_8192x5_tiled #(TILES=4) -> 32768 deep (LLaMA config)
//   payload: 5x sram_4096x34_tiled #(TILES=4) -> 16384 deep per slot bank
//            (LLaMA hottest slot 13696).  Every payload read activates exactly
//            one 4096x34 tile, the same per-read energy as the E2-2/E2-3/
//            E2-4/E2-5 LLaMA variants.
//
...
module exp1_candidate_a_pool_top_llama (
    input  wire         clk,
    input  wire         rst_n,
    ...
    // config / payload SRAM address widths (LLaMA: 32768-deep config from
    // 4x 8192x5 tiles, 16384-deep payload banks from 4x 4096x34 tiles)
    ...
    // config macro: 4x 8192x5 gated tiles -> 32768 deep
