...
// the deep program pools (BERT / SD-UNet / LLaMA).
//
...
module exp2_adaptive_4slot_pool_top #(
    parameter TILES_CFG = 1,   // config tiles (tiled caliber): BERT=1, SD-UNet=2
    parameter TILES_PAY = 1    // lane tiles (tiled caliber): BERT=1, SD-UNet=2
) (
    input  wire         clk,
    input  wire         rst_n,
...
`endif
    ...
    // payload lanes: MC 16384x34 (LLaMA) or tiled 4096x34
`ifdef PAY_MC
    ...
    sram_16384x34_wrapper u_lane3 (
        .clk(clk), .cen(lane3_cen), .wen(lane3_wen), .addr(lane3_addr),
        .wdata(load_wdata), .wmask(lane_wmask), .rdata(lane3_rdata));
`else
    sram_4096x34_tiled #(.TILES(TILES_PAY)) u_lane0 (
        .clk(clk), .cen(lane0_cen), .wen(lane0_wen), .addr(lane0_addr),
        .wdata(load_wdata), .wmask(lane_wmask), .rdata(lane0_rdata));
    sram_4096x34_tiled #(.TILES(TILES_PAY)) u_lane1 (
        .clk(clk), .cen(lane1_cen), .wen(lane1_wen), .addr(lane1_addr),
        .wdata(load_wdata), .wmask(lane_wmask), .rdata(lane1_rdata));
    sram_4096x34_tiled #(.TILES(TILES_PAY)) u_lane2 (
        .clk(clk), .cen(lane2_cen), .wen(lane2_wen), .addr(lane2_addr),
        .wdata(load_wdata), .wmask(lane_wmask), .rdata(lane2_rdata));
    sram_4096x34_tiled #(.TILES(TILES_PAY)) u_lane3 (
        .clk(clk), .cen(lane3_cen), .wen(lane3_wen), .addr(lane3_addr),
        .wdata(load_wdata), .wmask(lane_wmask), .rdata(lane3_rdata));
`endif
