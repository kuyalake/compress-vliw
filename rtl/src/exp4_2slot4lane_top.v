...
// -----------------------------------------------------------------------------
// E4: 2slot4lane instruction supply (rtl/plan.md section 5.5)
//
// Storage: config 1x sram_8192x6 (6-bit {group, local} code per cycle, T
// entries incl. the in-band EOP code 63) + payload 4x sram_2048x34 (shared
// lanes 0..3, grouped as two pairs {0,1} and {2,3}).
//
// Config semantics: code[5]=group selects the lane pair, code[4:0]=local is
// decoded by the SAME cfg_pair_codebook as E3 into {valid0, slot0, valid1,
// slot1}; local lane j of the pair carries slot_j when valid_j, i.e. physical
// lane (2*group + j). Code 63 = EOP (local code 31 then decodes all-invalid,
// so no lane is read); code 31 is reserved and never emitted.
//
// Pipeline (II=1, 3-cycle latency), config one stage ahead of payload:
//   stage A: config address = cfg_pc (registered) -> config macro;
//   stage B: config Q = code(k) -> {group, codebook(local)} -> lane enables
//            en(2g+j) = valid_j & (group==g) -> lane CENs/addresses
//            (critical path 1: config Q -> LUT+group -> payload CEN/A).
//            Registered at end of B: {valid0_c, slot0_c, valid1_c, slot1_c,
//            group_c} so stage C is pure muxing; lane pointers advance by en;
//   stage C: payload Q -> per-slot 4:1 mux (select from registered control)
//            -> field output registers (hold semantics, plan.md section 3.4).
//
// Hold-only output stage (no HOLD_EN parameter, per 2026-09-06 decision).
// Program pool: descriptor {cfg_base, prog_len, lane_base0..3} latched at
// start; lane pointers start at the lane bases. No backpressure.
// -----------------------------------------------------------------------------

module exp4_2slot4lane_top (
    input  wire         clk,
    input  wire         rst_n,
    ...
    input  wire [33:0]  load_wdata,   // config uses [5:0]
    // run control: program descriptor
    input  wire         start,
    input  wire [12:0]  cfg_base,
    input  wire [13:0]  prog_len,     // total cycles T (including the EOP entry)
    input  wire [10:0]  lane_base0,
    input  wire [10:0]  lane_base1,
    input  wire [10:0]  lane_base2,
    input  wire [10:0]  lane_base3,
    // LOAD slot fields
    ...
    // ------------------------------------------------------------------
    // Stage B: {group, codebook(local)} -> lane enables; pointers; ctrl regs
    // ------------------------------------------------------------------
    reg  [10:0] ptr0, ptr1, ptr2, ptr3;
    reg         c_vld;
    reg         c_eop;
    ...
    wire [5:0]  cfg_rdata;
    wire        is_eop_w = (cfg_rdata == 6'd63);
    wire        group_w  = cfg_rdata[5];

    // script-generated codebook decodes the local 5-bit field
    ...
    // local lane j of pair `group` -> physical lane 2*group + j.
    // (EOP: local code 31 decodes all-invalid, so no lane is read.)
    wire        act   = b_vld & ~is_eop_w;
    wire        en0   = act & valid0_w & ~group_w;  // lane 0 = pair 0 local 0
    wire        en1   = act & valid1_w & ~group_w;  // lane 1 = pair 0 local 1
    wire        en2   = act & valid0_w &  group_w;  // lane 2 = pair 1 local 0
    wire        en3   = act & valid1_w &  group_w;  // lane 3 = pair 1 local 1

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ptr0 <= 11'd0; ptr1 <= 11'd0; ptr2 <= 11'd0; ptr3 <= 11'd0;
            ...
            if (b_vld) begin
                ...
                slot1_c  <= slot1_w;
                group_c  <= group_w;
                if (en0) ptr0 <= ptr0 + 11'd1;
                if (en1) ptr1 <= ptr1 + 11'd1;
                if (en2) ptr2 <= ptr2 + 11'd1;
                if (en3) ptr3 <= ptr3 + 11'd1;
            end else begin
                valid0_c <= 1'b0; slot0_c <= 3'd0;
                valid1_c <= 1'b0; slot1_c <= 3'd0;
                group_c  <= 1'b0;
    ...
    wire [5:0]  cfg_wdata = load_wdata[5:0];
    wire [5:0]  cfg_wmask = 6'b111111;
    ...
    wire [10:0] lane0_addr = load_en ? load_addr[10:0] : ptr0;
    wire [10:0] lane1_addr = load_en ? load_addr[10:0] : ptr1;
    wire [10:0] lane2_addr = load_en ? load_addr[10:0] : ptr2;
    wire [10:0] lane3_addr = load_en ? load_addr[10:0] : ptr3;
    wire [33:0] lane_wmask = {34{1'b1}};
    wire [33:0] lane0_rdata, lane1_rdata, lane2_rdata, lane3_rdata;

    sram_8192x6_wrapper u_cfg (
        .clk   (clk),
        .cen   (cfg_cen),
        .wen   (cfg_wen),
        .addr  (cfg_addr),
    ...
    // ------------------------------------------------------------------
    // Stage C: per-slot 4:1 mux (from registered control) + hold
    // ------------------------------------------------------------------
    // local lane j hits slot s iff valid_j_c && slot_j_c == s; the source
    // physical lane is 2*group_c + j (codebook: slot0_c != slot1_c when both
    // valid, so no slot is driven by both local lanes)
    wire [33:0] data0 = lane0_rdata;
    wire [33:0] data1 = lane1_rdata;
    wire [33:0] data2 = lane2_rdata;
    wire [33:0] data3 = lane3_rdata;

    // hit on local lane 0 / local lane 1 for each slot
    wire h00 = valid0_c & (slot0_c == 3'd0);
    wire h01 = valid0_c & (slot0_c == 3'd1);
    wire h02 = valid0_c & (slot0_c == 3'd2);
    wire h03 = valid0_c & (slot0_c == 3'd3);
    wire h04 = valid0_c & (slot0_c == 3'd4);
    wire h10 = valid1_c & (slot1_c == 3'd0);
    wire h11 = valid1_c & (slot1_c == 3'd1);
    wire h12 = valid1_c & (slot1_c == 3'd2);
    wire h13 = valid1_c & (slot1_c == 3'd3);
    wire h14 = valid1_c & (slot1_c == 3'd4);

    wire        sel_load   = h00 | h10;
    wire        sel_store  = h01 | h11;
    wire        sel_vector = h02 | h12;
    wire        sel_scalar = h03 | h13;
    wire        sel_sfu    = h04 | h14;

    // source lane index: local lane 0 -> {group,1'b0}, local lane 1 -> {group,1'b1}
    wire [1:0]  src_load   = h00 ? {group_c, 1'b0} : {group_c, 1'b1};
    wire [1:0]  src_store  = h01 ? {group_c, 1'b0} : {group_c, 1'b1};
    wire [1:0]  src_vector = h02 ? {group_c, 1'b0} : {group_c, 1'b1};
    wire [1:0]  src_scalar = h03 ? {group_c, 1'b0} : {group_c, 1'b1};
    wire [1:0]  src_sfu    = h04 ? {group_c, 1'b0} : {group_c, 1'b1};

    wire [33:0] sc_load   = (src_load   == 2'd0) ? data0 : (src_load   == 2'd1) ? data1 :
                            (src_load   == 2'd2) ? data2 : data3;
    wire [33:0] sc_store  = (src_store  == 2'd0) ? data0 : (src_store  == 2'd1) ? data1 :
                            (src_store  == 2'd2) ? data2 : data3;
    wire [33:0] sc_vector = (src_vector == 2'd0) ? data0 : (src_vector == 2'd1) ? data1 :
                            (src_vector == 2'd2) ? data2 : data3;
    wire [33:0] sc_scalar = (src_scalar == 2'd0) ? data0 : (src_scalar == 2'd1) ? data1 :
                            (src_scalar == 2'd2) ? data2 : data3;
    wire [33:0] sc_sfu    = (src_sfu    == 2'd0) ? data0 : (src_sfu    == 2'd1) ? data1 :
                            (src_sfu    == 2'd2) ? data2 : data3;
