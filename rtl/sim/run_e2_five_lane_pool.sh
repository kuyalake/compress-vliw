#!/usr/bin/env bash
# E2 five-lane per-pool regression (plan e2_five_lane_detailed_experiment_plan).
...
#
# Usage:  bash rtl/sim/run_e2_five_lane_pool.sh [bert|sd_unet|llama|all]
set -euo pipefail
...
run_one() {  # suffix  pool_name  rev(0/1)
    ...
    local top="$RTL_DIR/src/exp2_five_lane_pool_top_${sfx}.v"
    local tb="$RTL_DIR/sim/tb_exp2_five_lane_pool_${sfx}.v"
    local srcs
    case "$sfx" in
        bert)
            srcs=( "$RTL_DIR/src/sram_8192x5_wrapper.v" "$RTL_DIR/src/sram_2048x34_wrapper.v"
                   "${M_8192x5[@]}" "${M_2048x34[@]}" ) ;;
        sd_unet)
    ...
    "$IV" -g2012 -P "tb_exp2_five_lane_pool_${sfx}.REV=$rev" -o "/tmp/e25_${sfx}_${rev}.vvp" \
        "$tb" "$top" "${srcs[@]}"
    ( cd "$REPO_DIR" && "$VVP" "/tmp/e25_${sfx}_${rev}.vvp" 2>&1 | grep -E "\[tb\]" || true )
    python3 "$RTL_DIR/tools/check_rtl_output.py" --pool \
        --rtl "$REPO_DIR/rtl_out_e2_5_${pname}${suffix}.txt" \
        --map "$RTL_DIR/data_model_pools/${pname}/e2_5_map.txt" \
        --data "$RTL_DIR/data_model_pools/${pname}/golden" --golden-name golden_issue5.txt
    rm -f "$REPO_DIR/rtl_out_e2_5_${pname}${suffix}.txt"
    rm -f "/tmp/e25_${sfx}_${rev}.vvp"
}
