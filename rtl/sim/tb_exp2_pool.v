...
// -----------------------------------------------------------------------------
// E2 model-pool tb (adaptive 4-slot, deep program pool). No plusargs.
//   cwd = repo root. Reads rtl/data_model_pools/<pool>/e2_*.memh; writes
//   rtl_out_e2_<pool>.txt with "# prog <k>" segment markers.
...
module tb_exp2_pool;
    ...
    exp2_adaptive_4slot_pool_top #(.TILES_CFG(TILES_CFG), .TILES_PAY(TILES_PAY)) dut (
        .clk           (clk),
        .rst_n         (rst_n),
        .load_en       (load_en),
        .load_sel      (load_sel),
    ...
    initial begin
        ...
        case (POOL_ID)
            0: out_fd = $fopen("rtl_out_e2_BERT.txt", "w");
            1: out_fd = $fopen("rtl_out_e2_SD-UNet.txt", "w");
            2: out_fd = $fopen("rtl_out_e2_LLaMA.txt", "w");
            default: out_fd = $fopen("rtl_out_e2.txt", "w");
        endcase
        if (out_fd == 0) begin
        ...
        $fclose(out_fd);
        $display("[tb] E2 POOL id=%0d programs=%0d cfg_reads=%0d lanes=%0d/%0d/%0d/%0d errors=%0d",
                 POOL_ID, n_prog, cfg_reads,
                 lane_reads[0], lane_reads[1], lane_reads[2], lane_reads[3],
                 pool_errors);
        if (pool_errors == 0) begin
            $display("[tb] PASS: E2 pool run completed, %0d programs", n_prog);
        end else begin
            $display("[tb] FAIL: E2 pool run had %0d errors", pool_errors);
        end
        $finish;
    end
