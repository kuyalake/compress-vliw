...
// -----------------------------------------------------------------------------
// E1 BERT-pool tb for exp1_candidate_a_pool_top_bert (Candidate A vertical
// compression, 5 fixed slot banks of 4096 depth built from 2x 2048x34 gated
// tiles).  No plusargs.
//   cwd = repo root.  Reads rtl/data_model_pools/BERT/e1_*.memh; writes
//   rtl_out_e1_BERT.txt (REV=0) / rtl_out_e1_BERT_rev.txt (REV=1) with
...
module tb_exp1_candidate_a_pool_bert;
    ...
    exp1_candidate_a_pool_top_bert dut (
        .clk           (clk),
        .rst_n         (rst_n),
        .load_en       (load_en),
        .load_sel      (load_sel),
    ...
    // the tiled 2048x34 construction as well)
    ...
    initial begin
        ...
        for (p = 0; p < 5; p = p + 1) begin
            if (pay_total[p] > PAY_DEPTH) begin
                $display("ERROR: BERT bank %0d depth %0d exceeds PAY_DEPTH %0d",
        ...
        out_fd = $fopen(REV ? "rtl_out_e1_BERT_rev.txt" : "rtl_out_e1_BERT.txt", "w");
        if (out_fd == 0) begin
        ...
        $fclose(out_fd);
        $display("[tb] E1 POOL BERT programs=%0d cfg_reads=%0d pay=%0d/%0d/%0d/%0d/%0d errors=%0d",
                 n_prog, cfg_reads,
                 pay_reads[0], pay_reads[1], pay_reads[2], pay_reads[3], pay_reads[4],
                 pool_errors);
        if (pool_errors == 0) begin
            $display("[tb] PASS: E1 pool BERT run completed, %0d programs", n_prog);
        end else begin
            $display("[tb] FAIL: E1 pool BERT run had %0d errors", pool_errors);
        end
        $finish;
    end
