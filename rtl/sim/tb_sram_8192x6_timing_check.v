...
module tb_sram_8192x6_timing_check;
    ...
    initial begin
        $dumpfile("sram_8192x6_timing_check.vcd");
        $dumpvars(0, tb_sram_8192x6_timing_check);
    end
    ...
    initial begin
        ...
        if (errors == 0 && read_checks == BURST_LEN) begin
            $display("SRAM 8192x6 continuous write/read simulation PASSED at 1GHz.");
        end else begin
            $display("SRAM 8192x6 continuous write/read simulation FAILED, errors=%0d read_checks=%0d.", errors, read_checks);
        end
        $finish;
    end
