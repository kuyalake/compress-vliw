#!/usr/bin/env bash
# LLaMA-pool lane-sweep regression under the UNIFORM gated-tile construction
# (fairness fix, 2026-09-16): every scheme uses config = sram_8192x5_tiled#4
# and payload built from 4096x34 tiles with per-tile CEN gating, so both a
# config read and a payload read cost the same energy in every scheme.
# Each scheme has its own dedicated top + tb (no generic tops):
#
#   scheme  top                                     payload construction
#   E1      exp1_candidate_a_pool_top_llama         5x sram_4096x34_tiled#4 (16384/bank)
#   E2-5    exp2_five_lane_pool_top_llama           5x sram_4096x34_tiled#2 (8192/lane)
#   E2-4    exp2_adaptive_4slot_pool_top_llama      4x sram_4096x34_tiled#4 (16384/lane)
#   E2-3    exp2_three_lane_pool_top_llama          3x sram_4096x34_tiled#4 (16384/lane)
#   E2-2    exp2_two_lane_pool_top_llama            2x sram_4096x34_tiled#8 (32768/lane)
#
...
#
# Usage:  bash rtl/sim/run_llama_lane_fair.sh [e1|e2_5|e2_4|e2_3|e2_2|all]
set -euo pipefail
...
run_one() {  # tag  tb_module  top_file  tb_file  golden_name  map_file  rev
    ...
    "$IV" -g2012 -P "${tbm}.REV=$rev" -o "/tmp/lfair_${tag}_${rev}.vvp" \
        "$RTL_DIR/sim/$tb" "$RTL_DIR/src/$top" "${SRCS[@]}"
    ( cd "$REPO_DIR" && "$VVP" "/tmp/lfair_${tag}_${rev}.vvp" 2>&1 | grep -E "\[tb\]" || true )
    python3 "$RTL_DIR/tools/check_rtl_output.py" --pool \
        --rtl "$REPO_DIR/rtl_out_${tag}_LLaMA${suffix}.txt" \
        --map "$RTL_DIR/data_model_pools/LLaMA/${map}" \
        --data "$RTL_DIR/data_model_pools/LLaMA/golden" --golden-name "$golden"
    rm -f "$REPO_DIR/rtl_out_${tag}_LLaMA${suffix}.txt"
    rm -f "/tmp/lfair_${tag}_${rev}.vvp"
}
...
case "$WHICH" in
    e1)
        run_scheme e1   tb_exp1_candidate_a_pool_llama \
            exp1_candidate_a_pool_top_llama.v tb_exp1_candidate_a_pool_llama.v \
            golden_issue5.txt e1_map.txt ;;
    e2_5)
        run_scheme e2_5 tb_exp2_five_lane_pool_llama \
            exp2_five_lane_pool_top_llama.v tb_exp2_five_lane_pool_llama.v \
            golden_issue5.txt e2_5_map.txt ;;
    e2_4)
        run_scheme e2   tb_exp2_pool_llama \
            exp2_adaptive_4slot_pool_top_llama.v tb_exp2_pool_llama.v \
            golden_issue4.txt e2_map.txt ;;
    e2_3)
        run_scheme e2_3 tb_exp2_three_lane_pool_llama \
            exp2_three_lane_pool_top_llama.v tb_exp2_three_lane_pool_llama.v \
            golden_issue3.txt e2_3_map.txt ;;
    e2_2)
        run_scheme e2_2 tb_exp2_two_lane_pool_llama \
            exp2_two_lane_pool_top_llama.v tb_exp2_two_lane_pool_llama.v \
