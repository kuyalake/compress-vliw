...
// -----------------------------------------------------------------------------
// Unit tb for sram_4096x34_tiled (depth-tiled payload SRAM).
// Same coverage as tb_sram_8192x5_tiled: full-depth continuous read-back,
// tile-boundary bursts, single-tile gating assertion, EOP-at-boundary stop.
// TILES overridden at compile time (iverilog -P tb_sram_4096x34_tiled.TILES=4).
// -----------------------------------------------------------------------------

module tb_sram_4096x34_tiled;

    parameter TILES = 2;
    localparam integer BASE_AW    = 12;
    localparam integer DW         = 34;
    localparam integer BASE_DEPTH = 4096;
    ...
    sram_4096x34_tiled #(.TILES(TILES)) dut (
        .clk   (clk),
        .cen   (in_valid),
        .wen   (in_write),
        .addr  (in_addr),
    ...
    function [DW-1:0] data_pattern;
        ...
        begin
            for (bit_idx = 0; bit_idx < DW; bit_idx = bit_idx + 1) begin
                data_pattern[bit_idx] = ((a * 7 + bit_idx * 3 + bit_idx / 4) & 1);
            end
        end
    endfunction
    ...
    initial begin
        ...
        if (errors == 0 && gating_errors == 0 &&
            ...
            $display("SRAM 4096x34 tiled (TILES=%0d, depth=%0d) PASSED at 1GHz. reads=%0d",
                     TILES, DEPTH, read_checks);
        end else begin
            $display("SRAM 4096x34 tiled (TILES=%0d) FAILED: errors=%0d gating_errors=%0d reads=%0d",
