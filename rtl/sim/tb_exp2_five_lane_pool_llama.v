...
// -----------------------------------------------------------------------------
// E2 five-lane model-pool tb.  No plusargs.
//   cwd = repo root.  Reads rtl/data_model_pools/<pool>/e2_5_*.memh; writes
//   rtl_out_e2_5_<pool>.txt with "# prog <k>" segment markers.
//   POOL_ID: 0=BERT, 1=SD-UNet, 2=LLaMA.
//   DUT: exp2_five_lane_pool_top (config sram_32768x5 + 5x sram_4096x34_tiled#2).
//   Implicit EOP: config holds prog_len-1 real entries; the last output cycle
//   is a hardware-generated all-NOP + eop.
// -----------------------------------------------------------------------------

module tb_exp2_five_lane_pool_llama;
    ...
    localparam integer LANE_DEPTH    = 8192;
    ...
    exp2_five_lane_pool_top_llama dut (
        .clk           (clk),
        .rst_n         (rst_n),
        .load_en       (load_en),
        .load_sel      (load_sel),
    ...
    reg  [33:0]   lane4_imem [0:LANE_DEPTH-1];
    reg  [31:0]   pool_meta [0:255];
    reg  [1023:0] pool_dir;
    integer       out_fd;
    integer       i;
    integer       p;
    integer       n_prog;
    integer       cfg_total;
    integer       lane_total [0:4];
    integer       emit_count;
    integer       cfg_reads;
    integer       lane_reads [0:4];
    integer       timeout;
    integer       pool_errors;
    ...
    always @(posedge clk) begin
        ...
        if (rst_n && dut.lane4_cen && !dut.lane4_wen) lane_reads[4] = lane_reads[4] + 1;
    end
    ...
    initial begin
        pool_dir = "rtl/data_model_pools/LLaMA";
        $readmemh({pool_dir, "/e2_5_meta.hex"}, pool_meta);
        n_prog        = pool_meta[0];
        ...
        lane_total[4] = pool_meta[6];
        $readmemh({pool_dir, "/e2_5_config5.memh"}, cfg_imem, 0, cfg_total-1);
        $readmemh({pool_dir, "/e2_5_lane0.memh"}, lane0_imem, 0, lane_total[0]-1);
        $readmemh({pool_dir, "/e2_5_lane1.memh"}, lane1_imem, 0, lane_total[1]-1);
        $readmemh({pool_dir, "/e2_5_lane2.memh"}, lane2_imem, 0, lane_total[2]-1);
        $readmemh({pool_dir, "/e2_5_lane3.memh"}, lane3_imem, 0, lane_total[3]-1);
        $readmemh({pool_dir, "/e2_5_lane4.memh"}, lane4_imem, 0, lane_total[4]-1);

        out_fd = $fopen(REV ? "rtl_out_e2_5_LLaMA_rev.txt" : "rtl_out_e2_5_LLaMA.txt", "w");
        if (out_fd == 0) begin
        ...
        emit_count  = 0;
        cfg_reads   = 0;
        lane_reads[0] = 0; lane_reads[1] = 0; lane_reads[2] = 0;
        lane_reads[3] = 0; lane_reads[4] = 0;
        pool_errors = 0;
        rst_n       = 1'b0;
        load_en     = 1'b0;
        load_sel    = 3'd0;
        ...
        cfg_base    = 15'd0;
        prog_len    = 15'd0;
        lane_base0  = 14'd0; lane_base1 = 14'd0; lane_base2  = 14'd0;
        lane_base3  = 14'd0; lane_base4  = 14'd0;
        ...
        do_load_macro(3'd5, lane_total[4]);
        load_en = 1'b0;

        // run programs one by one (REV=1: reverse storage order, plan 9.4)
        for (i = 0; i < n_prog; i = i + 1) begin
            p = REV ? (n_prog - 1 - i) : i;
            $fdisplay(out_fd, "# prog %0d", p);
            do_reset;
            cfg_base   = pool_meta[7 + 7*p][14:0];
            prog_len   = pool_meta[7 + 7*p + 1][14:0];
            lane_base0 = pool_meta[7 + 7*p + 2][13:0];
            lane_base1 = pool_meta[7 + 7*p + 3][13:0];
            lane_base2 = pool_meta[7 + 7*p + 4][13:0];
            lane_base3 = pool_meta[7 + 7*p + 5][13:0];
            lane_base4 = pool_meta[7 + 7*p + 6][13:0];
            emit_count = 0;
            @(negedge clk);
            start = 1'b1;
            @(negedge clk);
        ...
        $fclose(out_fd);
        $display("[tb] E2-5 POOL LLaMA programs=%0d cfg_reads=%0d lanes=%0d/%0d/%0d/%0d/%0d errors=%0d",
                 n_prog, cfg_reads,
                 lane_reads[0], lane_reads[1], lane_reads[2], lane_reads[3],
                 lane_reads[4], pool_errors);
        if (pool_errors == 0) begin
            $display("[tb] PASS: E2-5 pool LLaMA run completed, %0d programs", n_prog);
        end else begin
            $display("[tb] FAIL: E2-5 pool LLaMA run had %0d errors", pool_errors);
        end
        $finish;
    end
