...
module tb_exp2_two_lane_pool_sd_unet;
    ...
    exp2_two_lane_pool_top_sd_unet dut (
        .clk           (clk),
        .rst_n         (rst_n),
        .load_en       (load_en),
        .load_sel      (load_sel),
    ...
    initial begin
        ...
        out_fd = $fopen(REV ? "rtl_out_e2_2_SD-UNet_rev.txt" : "rtl_out_e2_2_SD-UNet.txt", "w");
        if (out_fd == 0) begin
        ...
        $fclose(out_fd);
        $display("[tb] E2-2 POOL SD-UNet programs=%0d cfg_reads=%0d lanes=%0d/%0d errors=%0d",
                 n_prog, cfg_reads,
                 lane_reads[0], lane_reads[1], pool_errors);
        if (pool_errors == 0) begin
            $display("[tb] PASS: E2-2 pool SD-UNet run completed, %0d programs", n_prog);
        end else begin
            $display("[tb] FAIL: E2-2 pool SD-UNet run had %0d errors", pool_errors);
        end
        $finish;
    end
