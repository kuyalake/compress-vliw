#!/usr/bin/env bash
# E4 regression under ModelSim/Questa: program-pool mode, hold-only.
# Requires vlib/vlog/vsim and python3 on PATH.
# Usage:  bash rtl/sim/run_e4_modelsim.sh
set -euo pipefail

RTL_DIR="$(cd "$(dirname "$0")/.." && pwd)"
BUILD="$RTL_DIR/out/modelsim_build_e4"
MODELS=(
...
vsim -c work.tb_exp4_2slot4lane \
    "+DATA=$RTL_DIR/data" \
    "+OUT=$pool_out_dir/rtl_out_e4_pool.txt" \
    -do "run -all; quit -f"
