#!/usr/bin/env bash
# E0 regression under ModelSim/Questa (vlog/vsim). Mirrors run_e0.sh (iverilog).
# Requires vlib/vlog/vsim and python3 on PATH.
# Usage:  bash rtl/sim/run_e0_modelsim.sh
set -euo pipefail

RTL_DIR="$(cd "$(dirname "$0")/.." && pwd)"
BUILD="$RTL_DIR/out/modelsim_build"
CASES=(softmax_x64 layernorm_x64 gelu_x64)
MODELS=(
    "$RTL_DIR/src/ts1n28hpcphvtb8192x144m4swbasod_180a_ffg0p88v0p99v0c.v"
    "$RTL_DIR/src/ts1n28hpcphvtb8192x27m4swbasod_180a_ffg0p88v0p99v0c.v"
)

mkdir -p "$BUILD"
cd "$BUILD"
vlib work

# Functional run: UNIT_DELAY skips the macro timing checks (fast, quiet).
# For a timing-checked run: drop UNIT_DELAY, keep SRAM_TIMING_SIM_INPUT_DELAY
# (the top already routes macro inputs through 0.120 ns delay hooks).
vlog +define+UNIT_DELAY +define+SRAM_TIMING_SIM_INPUT_DELAY \
    "${MODELS[@]}" \
    "$RTL_DIR/src/sram_8192x171_wrapper.v" \
    "$RTL_DIR/src/exp0_uncompressed_top.v" \
    "$RTL_DIR/sim/tb_exp0_uncompressed.v"

for hold in 1 0; do
    for case in "${CASES[@]}"; do
        mkdir -p "$RTL_DIR/out/$case"
        if [ "$hold" = "1" ]; then
            golden="$RTL_DIR/data/$case/golden_issue5.txt"; tag=e0
        else
            golden="$RTL_DIR/data/$case/golden_raw_issue5.txt"; tag=e0_raw
        fi
        # note: parameter override is vsim -G (not iverilog's -P); no recompile
        vsim -c -GHOLD_EN=$hold work.tb_exp0_uncompressed \
            "+DATA=$RTL_DIR/data/$case" \
            "+OUT=$RTL_DIR/out/$case/rtl_out_${tag}.txt" \
            -do "run -all; quit -f"
        python3 "$RTL_DIR/tools/check_rtl_output.py" \
            --rtl "$RTL_DIR/out/$case/rtl_out_${tag}.txt" \
            --golden "$golden" --case "${case}(HOLD_EN=$hold)"
    done

    # ---- program-pool run ----
    pool_out_dir="$RTL_DIR/out/pool"
    mkdir -p "$pool_out_dir"
    if [ "$hold" = "1" ]; then rawflag=""; else rawflag="--raw"; fi
    vsim -c -GHOLD_EN=$hold work.tb_exp0_uncompressed \
        +POOL=1 "+DATA=$RTL_DIR/data" \
        "+OUT=$pool_out_dir/rtl_out_e0_pool_hold${hold}.txt" \
        -do "run -all; quit -f"
    python3 "$RTL_DIR/tools/check_rtl_output.py" --pool \
        --rtl "$pool_out_dir/rtl_out_e0_pool_hold${hold}.txt" \
        --map "$RTL_DIR/data/e0_pool_map.txt" --data "$RTL_DIR/data" $rawflag
done
