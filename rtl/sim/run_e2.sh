#!/usr/bin/env bash
# E2 regression: program-pool mode, hold-only, real TSMC 28nm macro models.
# Drives the simple tb (tb_exp2_simple.v) from the repo root.
# Usage:  bash rtl/sim/run_e2.sh
#         IVERILOG=/path/to/iverilog VVP=/path/to/vvp bash rtl/sim/run_e2.sh
set -euo pipefail
...
simv="/tmp/exp2_simple.vvp"
"$IV" -g2012 -o "$simv" \
    "$RTL_DIR/sim/tb_exp2_simple.v" \
