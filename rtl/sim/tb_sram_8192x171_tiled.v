...
// -----------------------------------------------------------------------------
// Unit tb for sram_8192x171_tiled (depth-tiled uncompressed instruction SRAM).
...
// TILES overridden at compile time (iverilog -P tb_sram_8192x171_tiled.TILES=4).
// -----------------------------------------------------------------------------

module tb_sram_8192x171_tiled;

    parameter TILES = 2;
    localparam integer BASE_AW    = 13;
    localparam integer DW         = 171;
    ...
    sram_8192x171_tiled #(.TILES(TILES)) dut (
        .clk   (clk),
        .cen   (in_valid),
        .wen   (in_write),
        .addr  (in_addr),
    ...
    function [DW-1:0] data_pattern;
        ...
        begin
            for (bit_idx = 0; bit_idx < DW; bit_idx = bit_idx + 1) begin
                data_pattern[bit_idx] = ((a * 17 + bit_idx * 5 + bit_idx / 3) & 1);
            end
        end
    endfunction
    ...
    initial begin
        ...
        if (errors == 0 && gating_errors == 0 &&
            ...
            $display("SRAM 8192x171 tiled (TILES=%0d, depth=%0d) PASSED at 1GHz. reads=%0d",
                     TILES, DEPTH, read_checks);
        end else begin
            $display("SRAM 8192x171 tiled (TILES=%0d) FAILED: errors=%0d gating_errors=%0d reads=%0d",
