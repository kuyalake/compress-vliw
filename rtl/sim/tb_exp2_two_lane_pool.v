...
// -----------------------------------------------------------------------------
// E2 two-lane model-pool tb.  No plusargs.
//   cwd = repo root.  Reads rtl/data_model_pools/<pool>/e2_2_*.memh; writes
//   rtl_out_e2_2_<pool>.txt with "# prog <k>" segment markers.
//   POOL_ID: 0=BERT, 1=SD-UNet, 2=LLaMA.
//   DUT: exp2_two_lane_pool_top (config sram_32768x5 +
//   2x sram_16384x34_tiled#2 -> 32768 deep).
//   Implicit EOP: config holds prog_len-1 real entries; the last output cycle
//   is a hardware-generated all-NOP + eop.
// -----------------------------------------------------------------------------

module tb_exp2_two_lane_pool;
    ...
    localparam integer LANE_DEPTH    = 32768;
    ...
    initial begin
        ...
        endcase
        $readmemh({pool_dir, "/e2_2_meta.hex"}, pool_meta);
        n_prog        = pool_meta[0];
        cfg_total     = pool_meta[1];
        lane_total[0] = pool_meta[2];
        lane_total[1] = pool_meta[3];
        $readmemh({pool_dir, "/e2_2_config5.memh"}, cfg_imem, 0, cfg_total-1);
        $readmemh({pool_dir, "/e2_2_lane0.memh"}, lane0_imem, 0, lane_total[0]-1);
        $readmemh({pool_dir, "/e2_2_lane1.memh"}, lane1_imem, 0, lane_total[1]-1);

        case (POOL_ID)
            0: out_fd = $fopen(REV ? "rtl_out_e2_2_BERT_rev.txt"  : "rtl_out_e2_2_BERT.txt", "w");
            1: out_fd = $fopen(REV ? "rtl_out_e2_2_SD-UNet_rev.txt" : "rtl_out_e2_2_SD-UNet.txt", "w");
            2: out_fd = $fopen(REV ? "rtl_out_e2_2_LLaMA_rev.txt"  : "rtl_out_e2_2_LLaMA.txt", "w");
            default: out_fd = $fopen("rtl_out_e2_2.txt", "w");
        endcase
        if (out_fd == 0) begin
        ...
        cfg_base    = 15'd0;
        prog_len    = 15'd0;
        lane_base0  = 15'd0; lane_base1 = 15'd0;
        ...
        for (i = 0; i < n_prog; i = i + 1) begin
            ...
            do_reset;
            cfg_base   = pool_meta[4 + 4*p][14:0];
            prog_len   = pool_meta[4 + 4*p + 1][14:0];
            lane_base0 = pool_meta[4 + 4*p + 2][14:0];
            lane_base1 = pool_meta[4 + 4*p + 3][14:0];
            emit_count = 0;
            @(negedge clk);
            start = 1'b1;
            @(negedge clk);
        ...
        $fclose(out_fd);
        $display("[tb] E2-2 POOL id=%0d programs=%0d cfg_reads=%0d lanes=%0d/%0d errors=%0d",
                 POOL_ID, n_prog, cfg_reads,
                 lane_reads[0], lane_reads[1], pool_errors);
        if (pool_errors == 0) begin
            $display("[tb] PASS: E2-2 pool run completed, %0d programs", n_prog);
        end else begin
            $display("[tb] FAIL: E2-2 pool run had %0d errors", pool_errors);
        end
        $finish;
    end
