...
// -----------------------------------------------------------------------------
// E2 LLaMA-pool tb for exp2_adaptive_4slot_pool_top_llama (four shared lanes,
// 16384 deep per lane built from 4x 4096x34 gated tiles; in-band EOP).
//   Reads rtl/data_model_pools/LLaMA/e2_*.memh and writes
//   rtl_out_e2_LLaMA.txt (REV=0) / rtl_out_e2_LLaMA_rev.txt (REV=1) with
//   "# prog <k>" segment markers.
//   REV: 1 = run programs in reverse storage order (out-of-order descriptor
//   switching, plan 9.4).
// -----------------------------------------------------------------------------

module tb_exp2_pool_llama;
    ...
    exp2_adaptive_4slot_pool_top_llama dut (
        .clk           (clk),
        .rst_n         (rst_n),
        .load_en       (load_en),
        .load_sel      (load_sel),
    ...
    initial begin
        ...
        for (p = 0; p < 4; p = p + 1) begin
            if (lane_total[p] > LANE_DEPTH) begin
                $display("ERROR: LLaMA lane %0d depth %0d exceeds LANE_DEPTH %0d",
        ...
        out_fd = $fopen(REV ? "rtl_out_e2_LLaMA_rev.txt" : "rtl_out_e2_LLaMA.txt", "w");
        if (out_fd == 0) begin
        ...
        $fclose(out_fd);
        $display("[tb] E2 LLaMA POOL programs=%0d cfg_reads=%0d lanes=%0d/%0d/%0d/%0d errors=%0d",
                 n_prog, cfg_reads,
                 lane_reads[0], lane_reads[1], lane_reads[2], lane_reads[3],
                 pool_errors);
        if (pool_errors == 0) begin
            $display("[tb] PASS: E2 LLaMA pool run completed, %0d programs", n_prog);
        end else begin
            $display("[tb] FAIL: E2 LLaMA pool run had %0d errors", pool_errors);
        end
        $finish;
    end
