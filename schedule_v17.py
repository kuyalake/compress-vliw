...
def encode_load_slot(mem_fmt, mem_addr, gpr_fmt, gpr_addr, optype):
    return ((mem_fmt & 0b11) << 40) | ((mem_addr & 0xFFFFFFF) << 12) | ((gpr_fmt & 0b11) << 10) | ((gpr_addr & 0xFF) << 2) | (optype & 0b11)
...
def schedule_dag_nodes(dag_nodes, max_active_state, num_ve = 256, mask_fifo=8, mode = 0, add_rqo_option0=32, add_rqo_option1=16, add_rqo_option2=16):
    """
    调度 DAG 中的节点，生成指令时间表。
...
def run_scheduler(
    ...
    schedule, ve_usage, scalar_usage, sfu_usage = schedule_dag_nodes(
        ...
        add_rqo_option0=add_rqo_option0, add_rqo_option1=add_rqo_option1, add_rqo_option2=add_rqo_option2
    )

    # 8. 输出调度结果文件
    # 8.1 schedule.txt (184-bit hex)
    with open(os.path.join(out_dir, "schedule.txt"), "w") as f:
        for row in schedule:
            bin_row = "".join(format(x, f"0{w}b") for x, w in zip(row, [42, 42, 34, 31, 31, 3, 1]))
            hex_str = hex(int(bin_row, 2))[2:].zfill(46)
            f.write(hex_str + "\n")

    # 8.2 各槽位的csv/txt文件
    slot_fields = {
        "slot_load.csv":     (["mem_fmt", "mem_addr", "gpr_fmt", "gpr_addr", "optype"], [2, 28, 2, 8, 2]),
        "slot_store.csv":    (["mem_fmt", "mem_addr", "gpr_fmt", "gpr_addr", "optype"], [2, 28, 2, 8, 2]),
