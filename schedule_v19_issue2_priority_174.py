...
schedule_v19_issue2_priority_174 is modified on schedule_v18_issue2
it uses five equal 34-bit functional slots and evaluates multiple priority policies
"""
import pandas as pd
import math
import os
...
LOAD_STORE_SLOT_BITS = 34
FUNCTIONAL_SLOT_BITS = 34
VLIW_BITS = 174
...
def configure_priority_metadata(
    dag_nodes,
    mode,
    add_rqo_option0,
    add_rqo_option1,
    ...
    """Attach architecture-aware latency and remaining-critical-path estimates."""
    sfu_base_latency = {
        "exp": 24,
        "ln": 22,
        "sin": 24,
    ...
    def estimated_latency(node):
        op = node.op.op_type
        if op in ["const", "root", "store"]:
            return 0
        if op == "load":
            return 3
        ...
        if op in SFU_OPCODES:
            base = sfu_base_latency.get(op, 19)
            if node.type == 1:
                capacities = {
                    0: add_rqo_option0,
                    1: add_rqo_option0 + add_rqo_option1,
                    2: add_rqo_option0 + add_rqo_option1 + add_rqo_option2,
                }
                return base + math.ceil(node.vector_len / capacities[mode])
            return base
        return 7

    for node in dag_nodes.values():
        node.estimated_latency = estimated_latency(node)
        node.priority_fanout = len(
            [child for child in node.child_list if child in dag_nodes]
        )

    memo = {}
    visiting = set()

    def bottom_level(index):
        if index in memo:
            return memo[index]
        if index in visiting:
            raise RuntimeError("Cycle detected while calculating DAG bottom level")
        visiting.add(index)
        node = dag_nodes[index]
        children = [child for child in node.child_list if child in dag_nodes]
        remaining = max((bottom_level(child) for child in children), default=0)
        visiting.remove(index)
        memo[index] = node.estimated_latency + remaining
        return memo[index]

    for index, node in dag_nodes.items():
        node.bottom_level = bottom_level(index)
...
def schedule_dag_nodes(dag_nodes, max_active_state, num_ve=256, mask_fifo=8, mode=0,
                       ...
                       max_issue_slots=2, priority_strategy="index"):
    """
    调度 DAG 中的节点，生成指令时间表。
    ...
    if priority_strategy not in priority_strategies:
        raise ValueError(
            f"Unknown priority_strategy={priority_strategy}; "
            f"expected one of {sorted(priority_strategies)}"
        )
    ...
    def infer_slot(node):
        ...
        if op in SFU_OPCODES:
            return "sfu"
        return "vector" if node.type == 1 else "scalar"

    def priority_score(node):
        latency = node.estimated_latency
        bottom_level = node.bottom_level
        fanout = node.priority_fanout
        age = t - first_ready_cycle.get(node.index, t)
        if priority_strategy == "index":
            return -node.index
        if priority_strategy == "shortest_latency":
            return -1000 * latency - node.index
        if priority_strategy == "latency":
            return 1000 * latency - node.index
        if priority_strategy == "critical_path":
            return 1000 * bottom_level + latency - node.index / 1000000
        if priority_strategy == "critical_fanout":
            return 1000 * bottom_level + 8 * fanout + latency - node.index / 1000000
        if priority_strategy == "critical_age":
            return (
                1000 * bottom_level
                + 8 * fanout
                + 2 * min(age, 128)
    ...
    def order_waiting(waiting):
        index_order = sorted(waiting, key=lambda node: node.index)
        window_strategies = {
            "window8_latency": (8, "latency"),
            "window16_latency": (16, "latency"),
            "window8_critical": (8, "critical"),
            "window16_critical": (16, "critical"),
        }
        if priority_strategy in window_strategies:
            window_size, metric = window_strategies[priority_strategy]
            candidates = index_order[:window_size]
            if metric == "latency":
                candidates.sort(
                    key=lambda node: (
                        node.estimated_latency,
                        node.priority_fanout,
                        -node.index,
                    ),
                    reverse=True,
                )
            ...
            return candidates + index_order[window_size:]

        if priority_strategy in {"pair_window8", "pair_window16"}:
            window_size = 8 if priority_strategy == "pair_window8" else 16
            candidates = index_order[:window_size]
            best_pair = None
            best_key = None
            for i, first in enumerate(candidates):
                for j, second in enumerate(candidates[i + 1:], start=i + 1):
                    if infer_slot(first) == infer_slot(second):
                        continue
                    # Preserve the stable construction order as a strong prior,
                    # then favor long-latency/fanout pairs within that window.
                    key = (
                        -4 * (i + j)
                        + first.estimated_latency
                        + second.estimated_latency
                        + 2 * (first.priority_fanout + second.priority_fanout),
                        -(i + j),
                        -max(i, j),
                        -first.index,
                        -second.index,
            ...
            if best_pair is None:
                return index_order
            selected = {best_pair[0].index, best_pair[1].index}
            return [best_pair[0], best_pair[1]] + [
                node for node in index_order if node.index not in selected
            ]

        ordered = sorted(waiting, key=priority_score, reverse=True)
        if priority_strategy != "pair_critical" or max_issue_slots != 2:
            return ordered

        # Pair-aware lookahead is deliberately bounded. Full resource legality is
        # still checked by the normal issue path below after the pair is ordered.
        candidates = ordered[:48]
        best_pair = None
        best_score = None
        for i, first in enumerate(candidates):
            first_slot = infer_slot(first)
            for second in candidates[i + 1:]:
                if infer_slot(second) == first_slot:
                    continue
                unlocked = len(set(first.child_list) | set(second.child_list))
                score = (
                    priority_score(first)
                    + priority_score(second)
                    + 16 * unlocked
                )
                tie_break = (-first.index, -second.index)
                key = (score, tie_break)
        ...
        selected = {best_pair[0].index, best_pair[1].index}
        return [best_pair[0], best_pair[1]] + [
            node for node in ordered if node.index not in selected
        ]
    ...
    while NodeList:
        ...
        for node in list(NodeList):
            ...
            if pred_issued:
                waiting.append(node)
                first_ready_cycle.setdefault(node.index, t)
        
        waiting = order_waiting(waiting)
...
def run_scheduler(
    ...
    - PRIORITY_STRATEGY: 发射候选节点排序/节点对启发式
    - out_dir: 输出文件保存目录
    返回值：
    ...
    # 8. 输出调度结果文件
    # 8.1 schedule.txt (174-bit VLIW, stored in 44 hexadecimal digits)
    with open(os.path.join(out_dir, "schedule.txt"), "w") as f:
        for row in schedule:
            slot_widths = [FUNCTIONAL_SLOT_BITS] * 5 + [3, 1]
...
if __name__ == "__main__":
    ...
    schedule, perf, cycles = run_scheduler(
        ...
        add_rqo_option2=16,
        MAX_ISSUE_SLOTS=2,
        PRIORITY_STRATEGY="pair_critical",
        out_dir="./out_v19_issue2_priority_174/"
    )