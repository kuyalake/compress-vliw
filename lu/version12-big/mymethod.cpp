    ...
    for (int i = 0; i < time; i++) {
        ...
        for (int j = NPE - 1; j >= 0; j--) {
            ...
            if (rs != -1 && (inst[i][j].op != 19) && (inst[i][j].op != 20))
            {
                ...
                strings[rs_bank] = std::bitset<12>(rs_addr).to_string();
            }
            if (rt != -1)
            {
                int rt_addr = rt / NBANK;
                int rt_bank = rt % NBANK;
                strings[rt_bank] = std::bitset<12>(rt_addr).to_string();
            }
            if (rd != -1)
            {
                int rd_addr = rd / NBANK;
                int rd_bank = rd % NBANK;
                strings[rd_bank] = std::bitset<12>(rd_addr).to_string();
            }
        }
        std::string bs2;
        for (int j = NBANK - 1; j >= 0; j--)
