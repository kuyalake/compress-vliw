...
void schedule2(InstPack** inst, std::vector<int>* ocp_rd[], std::vector<int>* ocp_wr[], EleNode* EleNodePointer[], 
    ...
    for (int j = 0; j < n; j++) // 只对前n个PE进行调度，后面n~NPE的PE是空的
    {
        ...
        if (ok_to_wb[j] == 1) // 找到一个空位j，但node[j]此时已经无操作了，只等写回
        {
            ...
            {
                //检查写回是否会有冲突        
                int ok1 = check(node[j], ocp_wr[i + 1]);
                if (!ok1) continue;
...
int simulate(InstPack** inst, std::vector<float>& Ax, int time, const int n_node) {
    ...
    for (int i = 0; i < time; i++) {
        for (int j = 0; j < NPE; j++) {
            ...
            else if (inst[i][j].op == 22)
            {
                Ax[inst[i][j].rs] = reg[j];
                std::cout << "time: " << i << "\tPE: " << j << "\top: ";
...
void writeInst(InstPack** inst, const int time, const std::string& f1, const std::string& f2, const std::string& f3, const std::string& f4, const std::string& f5) {
...
// generate wea.txt
    for (int i = 0; i < time; i++) {
        ...
        for(int j = NPE; j >= 0; j--) {
            int des_bank = -1;
            if (inst[i][j].op == 1 || inst[i][j].op == 3 || inst[i][j].op == 19 || inst[i][j].op == 20 || inst[i][j].op == 22)
...
// generate addr.txt
    for (int i = 0; i < time; i++) {
        ...
        for (int j = NPE - 1; j >= 0; j--) {
            ...
            if (rs != -1 && (inst[i][j].op != 19) && (inst[i][j].op != 20) && (inst[i][j].op != 22))
            {
...
//generate sel2.txt
    for (int i = 0; i < time; i++) {
        ...
        for (int j = 0; j < NPE; j++) {
            ...
            else if (inst[i][j].op == 1 || inst[i][j].op == 3 || inst[i][j].op == 19 || inst[i][j].op == 20 || inst[i][j].op == 22)
...
// generate sel1.txt
    for (int i = 0; i < time; i++)
    {
        ...
        for (int j = 0; j < NPE; j++)
        {
            if (inst[i][j].op == 0 || inst[i][j].op == 22) continue;
            int rs = inst[i][j].rs;
            int rt = inst[i][j].rt;
            int rd = inst[i][j].rd;
            // 排列顺序（rs,rt,rd)
