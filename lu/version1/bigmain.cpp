...
int main(int argc, char* argv[]) {
    ...
    int fill_ok = fillIn(CAx, CAi, CAp, n, nnz, CAx_fill, CAi_fill, CAp_fill, CAd_fill);   
    ...
    cout << "div + ms count is " << div_count + ms_count << " the ideal cycle is " << (div_count + 2 * ms_count)/NPE  + 1<< endl;
...
//    cout << "the pool is :\n";
//    EleNodePointer[11809]->printPool();
//    cout << "the size is : " << EleNodePointer[11809]->pool.size() << std::endl;
...
/*    for (int i = 0; i < nnz_fill; i++) {
    ...
    for (const auto& element : newly_valid) {
        std::cout << element << " ";
    }
    std::cout << endl;
*/
...
*/
    ...
    if ((time + 3) >= INSTNUM) {
        ...
        int simulate_ok = simulate(inst, CAx_fill, CAi_fill, INSTNUM);
    ...
    int simulate_ok = simulate(inst, CAx_fill, CAi_fill, time + 3);
    if (simulate_ok == 0) cout << "SUCCESS: simulation\n";
