...
void readMatrixMarket(const char* filename, vector<double>& CAx, vector<int>& CAi, vector<int>& CAp, int* p_n, long int* p_nnz) {
...
void cscToCsr(
    const std::vector<double>& CAx, // CSC存储的矩阵非零元素
    const std::vector<int>& CAi, // CSC存储的行索引
    const std::vector<int>& CAp, // CSC存储的列指针
    std::vector<double>& RAx,       // 转换后的CSR存储的矩阵非零元素
...
void printSparse(
    const std::vector<double>& Ax, // 非零元素
...
std::vector<double> extractSubVectorFloat(const std::vector<double>& original, int start, int end) {
...
void addMissingElements(const std::vector<int>& Ai, int start, int end, std::vector<int>& subAi, std::vector<double>& subAx, std::vector<int>& myQue) {
...
void addMissingElementsFalse(const std::vector<int>& Ai, int start, int end, std::vector<int>& subAi, std::vector<double>& subAx, std::vector<int>& myQue) {
    for (int i = start; i <= end; ++i) {
        if (std::find(subAi.begin(), subAi.end(), Ai[i]) == subAi.end()) {
            subAi.push_back(Ai[i]);
...
int fillIn(
    ...
    const std::vector<int>& Ap, // 指针
    const int n, //order
    std::vector<double>& Ax_fill, //非零元素after fill in
    ...
    std::vector<double> Ax_0 = extractSubVectorFloat(Ax, Ap[0], Ap[1] - 1);
    Ax_fill.insert(Ax_fill.end(), Ax_0.begin(), Ax_0.end());

    for (int k = 1; k < n; k++)
    {
        std::vector<int> subAi = extractSubVectorInt(Ai, Ap[k], Ap[k + 1] - 1);
        std::vector<double> subAx = extractSubVectorFloat(Ax, Ap[k], Ap[k + 1] - 1);
...
void writeCSCtoMTX(const std::vector<double>& Ax, const std::vector<int>& Ai, const std::vector<int>& Ap, const int numRows, const int numCols, const int numNonZero, const std::string& filename) {
...
int simulate(InstPack** inst, std::vector<double>& Ax, int time, const int n_node) {
    std::vector<double> regC(n_node, 0);
    std::vector<double> reg(n_node);
    ...
    for (int i = 0; i < time; i++) {
        for (int j = 0; j < NPE; j++) {
            ...
            {
                double A = Ax[inst[i][j].rs];
                double B = Ax[inst[i][j].rt];
                // A / B
                if (B == 0)
                {
...
void writeData(const std::vector<double>& Ax, const int nnz, const std::string& filename) {
    std::ofstream file(filename);
...
void compareGolden(const std::vector<double>& g, const std::vector<double>& my, const int nnz) {
    int correct = 0;
    int wrong = 0;
    for (int i = 0; i < nnz; i++) {
        if (g[i] == 0) {
