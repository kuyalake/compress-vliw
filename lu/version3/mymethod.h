...
public:
    ...
    void printPool() {
        for (OpPack element : pool) {
            std::cout << "<" << element.rs << "," << element.rt << "," << element.rd << "," << element.op << "," << element.diag << "> ";
        }
        std::cout << std::endl;
    }
...
void schedule(InstPack inst[][NPE], std::vector<int>* ocp[], EleNode* EleNodePointer[], std::vector<OpPack>& pool, int i, std::vector<int>& newly_valid, std::vector<int>& remain);
