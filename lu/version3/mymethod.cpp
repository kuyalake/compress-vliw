...
void schedule(InstPack inst[][NPE], std::vector<int>* ocp[], EleNode* EleNodePointer[], std::vector<OpPack>& pool, int i, std::vector<int>& newly_valid, std::vector<int>& remain_node) {
    for (int j = 0; j < NPE; j++) {
...
void schedule2(InstPack inst[][NPE], std::vector<int>* ocp[], EleNode* EleNodePointer[], 
                ...
                std::vector<std::vector<int>*>& ocp_wb, const int time, int* p_current) {
    ...
    for (int i = 0; i < time; i++) {
        for (int j = 0; j < NPE; j++) {
            if (!(inst[i][j].op == 2 || inst[i][j].op == 3 || (inst[i][j].sel4 && inst[i][j].op == 1)) && ((!inst[i][j].valid) || (!inst[i][j].sel3)))
                continue;
            if (!inst[i][j].sel4 && inst[i][j].op == 1) //是除法的最后一个周期
            {
