...
// generate mask.txt (validity bits: op 16b, sel1 48b, addr 32b; sel2 mask == wea.txt)
    std::ofstream file7("mask.txt");
    for (int i = 0; i < time; i++) {
        // op mask: PE j = NPE-1 .. 0
        std::string m_op;
        for (int j = NPE - 1; j >= 0; j--)
            m_op += (inst[i][j].op != 0) ? "1" : "0";
        // sel1 mask: field idx = 3*NPE-1 .. 0; idx=3j+2 -> rs, 3j+1 -> rt, 3j -> rd
        std::string m_sel1;
        for (int idx = 3 * NPE - 1; idx >= 0; idx--) {
            int j = idx / 3;
            int k = idx % 3;
            bool v = false;
            if (inst[i][j].op != 0) {
                if (k == 2 && inst[i][j].rs != -1 && inst[i][j].op != 19 && inst[i][j].op != 20) v = true;
                if (k == 1 && inst[i][j].rt != -1) v = true;
                if (k == 0 && inst[i][j].rd != -1) v = true;
            }
            m_sel1 += v ? "1" : "0";
        }
        // addr mask: bank b = NBANK-1 .. 0, replicate addr.txt read logic
        std::vector<std::string> astrings(NBANK);
        for (int b = NBANK - 1; b >= 0; b--) astrings[b] = "11111111111";
        for (int j = NPE - 1; j >= 0; j--) {
            if (inst[i][j].op == 0) continue;
            int rs = inst[i][j].rs;
            int rt = inst[i][j].rt;
        ...
        std::string m_addr;
        for (int b = NBANK - 1; b >= 0; b--)
            m_addr += (astrings[b] != "11111111111") ? "1" : "0";
        file7 << m_op << "_" << m_sel1 << "_" << m_addr << std::endl;
    }
    file7.close();
