...
void printInst(const InstPack inst[][NPE], const int start, const int end){
    ...
    for(int i = start; i < end; i++){
        ...
        for (int j = 0; j < NPE; j++){
            ...
            << ", " << inst[i][j].sel1 << ", " << inst[i][j].sel2 << ", " << inst[i][j].sel3 << ", " << inst[i][j].valid << ")";
        }
        std::cout << std::endl;
    }
...
int simulate(const InstPack inst[][NPE], std::vector<float>& Ax, int time) {
    for (int i = 0; i < time; i++) {
        for (int j = 0; j < NPE; j++) {
            if ((!inst[i][j].valid) || (!inst[i][j].sel3))
                continue;
            if (inst[i][j].op == 1) //是除法的最后一个周期
            {
