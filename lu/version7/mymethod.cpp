...
void schedule2(InstPack** inst, std::vector<int>* ocp_rd[], std::vector<int>* ocp_wr[], EleNode* EleNodePointer[], 
    ...
    for (int k = 0; k < n; k++)
    {
        int j = k % NPE; //计算每个ele对应的PE
        if (EleNodePointer[node[k]]->getValid()) //该元素已经操作完成
            continue;
        if (inst[i][j].op != 0)   // 对应PE已经被填充过了，跳过
            continue;
        if (ok_to_sub[k] == 1) 
        {
            if (regC_ready_time[k] > i) //受到mac操作影响，regC/mac_result还不能读。
                continue;
            // 检查读出被减数C是否会有冲突
            int ok1 = check(node[k], ocp_rd[i]);
            if (!ok1) continue;
            // 没有冲突，可以做减法。将该操作调度到i的PE j上
            inst[i][j].rs = node[k];
            inst[i][j].rt = k; //为了simulation加的
            if (regC_ready_time[k] == i) //mac结果直接连
                inst[i][j].op = 17;
            else //减数从regC中来
                inst[i][j].op =18;
            // 处理该操作带来的后果
            if (!nodes[k]->size()) //后面没有除法操作了，可以写回
            {
                ok_to_wb[k] = 1;
                ok_to_wb_time[k] = i + 4;
            }
            if (ok1 == 1) ocp_rd[i]->push_back(node[k]);
            //不用在nodes中删除操作
            ok_to_sub[k] = 0;
            continue;
        }

        if (ok_to_wb[k] == 1) // 找到一个空位j，但node[k]此时已经无操作了，只等写回
        {
            if (ok_to_wb_time[k] > i) //还没有准备好
                continue;
            else // 已经准备好了
            {
                //检查写回是否会有冲突        
                int ok1 = check(node[k], ocp_wr[i + 1]);
                if (!ok1) continue;
                ...
                inst[i][j].rt = k; //为了simulation加的
                // 处理该操作带来的后果
                EleNodePointer[node[k]]->setTime(i + 2);
                EleNodePointer[node[k]]->setValid(1);
                (*p_current)--;

                ocp_wr[i + 1]->push_back(node[k]);
                //不用在nodes中删除操作
        ...
        // 找到一个空位j，查看node[k]对应的第一个操作是否可以做
        std::vector<OpPack>::iterator it = nodes[k]->begin();
        // 首先判断it的操作类型
        if (it->op) //是除法
        {
            // 判定是否只有除法
            if (only_div[k]) // 只有一个除法
            {
                ...
                inst[i + 1][j].rs = k; //为了simulation加的
                inst[i + 1][j].rt = it->rt;
                inst[i + 1][j].op = 20;
        ...
        else // 是ms
        {
            ...
            if (it->rs % NBANK == it->rt % NBANK) // AB冲突，需要两个slot：i,i+1
            {
                ...
                if (!(EleNodePointer[it->rt]->getValid() && EleNodePointer[it->rs]->getValid()))
                    continue;
                // 判定regC是否ready
                if (regC_ready_time[k] > (i + 1)) //受到mac操作影响，regC/mac_result还不能读。
                    continue;
                // 判定操作数是否ready
                ...
                inst[i + 1][j].op = 14;
                inst[i + 1][j].rs = it->rs;
                inst[i + 1][j].rd = k; //为了simulation加的

                // 处理该操作带来的后果
                regC_ready_time[k] = i + 1 + 3;
                if ((nodes[k]->size() == 1) || (nodes[k]->size() == 2 && (it + 1)->op)) //这个m就是最后一次操作了，之后就s
                    ok_to_sub[k] = 1; // 可以减法
            ...
            else // AB未冲突，需要考虑上一个操作是否连续的情况
            {
                ...
                inst[i][j].rs = it->rs;
                inst[i][j].rt = it->rt;
                inst[i][j].rd = k; //为了simulation加的
                if (regC_ready_time[k] == i) //数据直连mac_result
                    inst[i][j].op = 15;
                else //数据从regC来
                    inst[i][j].op = 12;

                // 处理该操作带来的后果
                regC_ready_time[k] = i + 3;
                if ((nodes[k]->size() == 1) || (nodes[k]->size() == 2 && (it + 1)->op)) //这个m就是最后一次操作了，之后就s
                    ok_to_sub[k] = 1; // 可以减法
...
int simulate(InstPack** inst, std::vector<float>& Ax, int time, const int n_node) {
    ...
    for (int i = 0; i < time; i++) {
        for (int j = 0; j < NPE; j++) {
            ...
            else if (inst[i][j].op == 17 || inst[i][j].op == 18) //减法
            {
                float X;
                float Y;
                Y = regC[inst[i][j].rt]; //regC[k]
                X = Ax[inst[i][j].rs];
                reg[inst[i][j].rt] = X - Y;
                std::cout << "time: " << i << "\tPE: " << j << "\top: ";
                std::cout << X << " - " << Y << " = " << reg[inst[i][j].rt] << endl; 
                ele[inst[i][j].rt] = inst[i][j].rs;
            }
            else if (inst[i][j].op == 22) //regF[k]写回
            {
                Ax[inst[i][j].rs] = reg[inst[i][j].rt];
                std::cout << "time: " << i << "\tPE: " << j << "\top: ";
                std::cout << "Ax[" << inst[i][j].rs << "] = " << Ax[inst[i][j].rs] << endl; 
            }
            else if (inst[i][j].op == 21) //写入regF[k]
            {
                reg[inst[i][j].rt] = Ax[inst[i][j].rs];
                std::cout << "time: " << i << "\tPE: " << j << "\top: ";
                std::cout << "reg[" << inst[i][j].rt << "] = " << reg[inst[i][j].rt] << endl; 
                ele[inst[i][j].rt] = inst[i][j].rs; //ele[k]
            }
            else if (inst[i][j].op == 19 || inst[i][j].op == 20) //除法
            {
                float B = Ax[inst[i][j].rt];
                float A = reg[inst[i][j].rs];
                if (B == 0)
                {
                    std::cout << "divided by zero in inst[" << i << "][" << j << "]\n";
                    return -1;
                ...
                reg[inst[i][j].rs] = A / B;
                std::cout << "time: " << i << "\tPE: " << j << "\top: ";
                std::cout << "Ax[" << ele[inst[i][j].rs] << "] = " << A << " / " << B << " = " << reg[inst[i][j].rs] << endl; 
                Ax[ele[inst[i][j].rs]] = reg[inst[i][j].rs];
            }
            else if (inst[i][j].op == 12 || inst[i][j].op == 15) //ms未冲突
            {
                float C = regC[inst[i][j].rd];
                float A = Ax[inst[i][j].rs];
                float B = Ax[inst[i][j].rt];
                regC[inst[i][j].rd] = C + A * B;
                std::cout << "time: " << i << "\tPE: " << j << "\top: ";
                std::cout << C << " + " << A << " * " << B << " = " << regC[inst[i][j].rd] << endl; 
            }
