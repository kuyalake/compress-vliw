...
int main(int argc, char* argv[]) {
    ...
    std::vector<bool> pivot_checked(n, 0);
    ...
    for (int i = 0; i < nnz_fill; i++) {
        std::cout << "\nNode " << i << " - n: " << EleNodePointer[i]->getN() << ", ai: " << EleNodePointer[i]->getAi() << ", col: " << EleNodePointer[i]->getCol() << std::endl;
    ...
    schedule(inst, ocp, EleNodePointer, ok_to_do, time, newly_valid, remain_node, pivot_checked);
    ...
    for (time = 1; time < INSTNUM; time++) {
        ...
        if (remain_node.size() <= NPE) {
            for (int i = 0; i < nnz_fill; i++) {
                if (!EleNodePointer[i]->getToDo()) continue;
                std::cout << "\nNode " << i << " - n: " << EleNodePointer[i]->getN() << ", ai: " << EleNodePointer[i]->getAi() << std::endl;
                std::cout << "ready time: " << EleNodePointer[i]->getTime() << ", valid: " << EleNodePointer[i]->getValid() << std::endl;
                std::cout << "child: ";
                EleNodePointer[i]->printChild();
