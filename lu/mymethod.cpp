...
//            cout << "col is " << k << ", row is " << row << endl;
            ...
            EleNodePointer[ele]->setCol(k); //设置列
...
void schedule(InstPack inst[][NPE], std::vector<int>* ocp[], EleNode* EleNodePointer[], std::vector<OpPack>& pool, int i, std::vector<int>& newly_valid, std::vector<int>& remain_node, std::vector<bool>& pivot_checked) {
    for (int j = 0; j < NPE; j++) {
        ...
        for (std::vector<OpPack>::iterator it = pool.begin(); it != pool.end(); ++it) {
            ...
            if (it->op) //是除法
            {
                int col = EleNodePointer[it->rt]->getCol();
                if (pivot_checked[col]) 
                {
                    //判定除数和被除数是否冲突 rd rt
                    if (it->rd % NBANK == it->rt % NBANK) //冲突啦，需要两个连续的slot：i i+1
                    {
                        ...
                        inst[i + 1][j].valid = 1;
                        //处理该操作带来的后果
                        EleNodePointer[it->rd]->setTime(i + 1 + DELAY);
                    ...
                    else //没有冲突，只需要一个slot: i
                    {
                        ...
                        inst[i][j].valid = 1;
                        //处理该操作带来的后果
                        EleNodePointer[it->rd]->setTime(i + DELAY);
                        int remain = EleNodePointer[it->rd]->getToDo();
                        if (remain == 1)
                        {
                            EleNodePointer[it->rd]->setValid(1);
                ...
                else //之前未进行过比较，因此需要在真正处理该op前发送一个compare操作
                {
                    //尝试把compare调度到该周期
                    //判定操作数是否ready
                    if (EleNodePointer[it->rd]->getTime() > i)
                        continue;
                    int ok1 = check(it->rd, ocp[i]);
                    //检查bank是否冲突
                    if (!ok1) continue;
                    //可以将compare调度到i上
                    inst[i][j].rs = it->rd;
            ...
            else //是ms
            {
                ...
                if ((it->rd % NBANK == it->rs % NBANK) && (it->rd % NBANK == it->rt % NBANK)) //冲突啦，需要3个连续的slot：i i+1 i+2
                {
                    ...
                    inst[i + 2][j].valid = 1;
                    //处理该操作带来的后果
                    EleNodePointer[it->rd]->setTime(i + 2 + DELAY);
                    int remain = EleNodePointer[it->rd]->getToDo();
...
void printInst(const InstPack inst[][NPE], const int start, const int end){
    ...
    for(int i = start; i < end; i++){
        ...
        for (int j = 0; j < NPE; j++){
            ...
            << ", " << inst[i][j].sel1 << ", " << inst[i][j].sel2 << ", " << inst[i][j].sel3 << ", " << inst[i][j].sel4 << ", " << inst[i][j].valid << ")";
        }
        std::cout << std::endl;
    }
...
void schedule2(InstPack inst[][NPE], std::vector<int>* ocp[], EleNode* EleNodePointer[], 
                std::vector<std::vector<OpPack>*>& nodes, const std::vector<int>& node, const int n, 
                const int i, const std::vector<bool>& only_div, std::vector<bool>& ok_to_sub, std::vector<bool>& ok_to_wb, std::vector<int>& ok_to_wb_time, 
                std::vector<std::vector<int>*>& ocp_wb, std::vector<bool>& write_reg, std::vector<bool>& pivot_checked,
                const int time, int* p_current) {
    ...
    for (int j = 0; j < n; j++) // 只对前n个PE进行调度，后面n~NPE的PE是空的
    {
        if (EleNodePointer[node[j]]->getValid()) //该元素已经操作完成
            continue;
        if (inst[i][j].valid)   // 已经被填充过了，跳过
            continue;
        
        if (ok_to_sub[j] == 1) 
        {
            // 检查读出被减数C是否会有冲突
            int ok1 = check(node[j], ocp[i]);
            if (!ok1) continue;
            // 没有冲突，可以做减法。将该操作调度到i的PE j上
            inst[i][j].rd = node[j];
            inst[i][j].op = 2;
            if (!inst[i - 1][j].valid) inst[i][j].sel3 = 1;
            inst[i][j].valid = 1;
            // 处理该操作带来的后果
            if (!nodes[j]->size()) //后面没有除法操作了，可以写回
            {
                ok_to_wb[j] = 1;
                ok_to_wb_time[j] = i + 4;
            }
            if (ok1 == 1) ocp[i]->push_back(node[j]);
            //不用在nodes中删除操作
            ok_to_sub[j] = 0;
            continue;
        }

        if (ok_to_wb[j] == 1) // 找到一个空位j，但node[j]此时已经无操作了，只等写回
        {
            if (ok_to_wb_time[j] > i) //还没有准备好
                continue;
            else // 已经准备好了
            {
                //检查写回是否会有冲突        
                int ok1 = check(node[j], ocp_wb[i - time]);
                if (!ok1) continue;
                ...
                inst[i][j].valid = 1;
                // 处理该操作带来的后果
                EleNodePointer[node[j]]->setTime(i + 2);
                EleNodePointer[node[j]]->setValid(1);
                (*p_current)--;

                ocp_wb[i - time]->push_back(node[j]);
                //不用在nodes中删除操作
        ...
        {
            //判定reg中是否有数
            if (write_reg[j]) //已经通过200或3001写入了reg，后面执行除法操作即可
            {
                
            }
            else //先进行3001加载被除数到reg
            {
                //检查操作数rd是否ready
                if (EleNodePointer[it->rd]->getTime() > i)
                    continue;
            ...
            if (only_div[j]) // 只有一个除法
            {
                ...
                if (!ok1) continue;
                // 检查rt能否放入i+1
                int ok2 = check(it->rt, ocp[i + 1]);
                if (!ok2) continue;
                // 满足条件！ 将该操作调度到i和i+1的PE j上 3001
                inst[i][j].rd = it->rd;
                inst[i][j].op = 3;
                inst[i][j].sel3 = 1;
                inst[i][j].valid = 1;
                ...
                if (ok1 == 1) ocp[i]->push_back(it->rd);
                if (ok2 == 1) ocp[i + 1]->push_back(it->rt);
                nodes[j]->erase(it); // 在nodes中删除该操作
                            
            }
            else // 不止有一个除法，所以不用读入reg，因为被除数要么从sub-result来，要么sub-result的结果已经存到了reg中    
            {
                // 检查rt是否valid
                if (!EleNodePointer[it->rt]->getValid())
                    continue;
                ...
                inst[i][j].rt = it->rt;
                inst[i][j].op = 1;
                if (!inst[i - 1][j].valid) //操作不连续，被除数从reg中来；否则被除数直接连sub-result
                    inst[i][j].sel1 = 1;
                inst[i][j].sel2 = 1;
                inst[i][j].sel4 = 1;
                inst[i][j].valid = 1;
        ...
        else // 是ms
        {
            ...
            else // AB未冲突，需要考虑上一个操作是否连续的情况
            {
                ...
                inst[i][j].sel1 = 1;
                inst[i][j].sel2 = 1;
                if (!(inst[i - 1][j].valid && inst[i - 1][j].op == 2)) inst[i][j].sel3 = 1;
                inst[i][j].sel4 = 1;
                inst[i][j].valid = 1;
                // 处理该操作带来的后果
                if ((nodes[j]->size() == 1) || (nodes[j]->size() == 2 && (it + 1)->op)) //这个m就是最后一次操作了，之后就s
...
int simulate(const InstPack inst[][NPE], std::vector<float>& Ax, int time, const int n_node) {
//    std::vector<bool> change_to_sch2(n_node, 0); //标志已经进入sch2了。由第一次检测到op = 2或3时完成转变。
    std::vector<float> regC(n_node, 0);
    std::vector<float> reg(n_node);

    for (int i = 0; i < time; i++) {
        for (int j = 0; j < NPE; j++) {
            if (!(inst[i][j].op == 2 || inst[i][j].op == 3 || (inst[i][j].sel4 && inst[i][j].op == 1)) && ((!inst[i][j].valid) || (!inst[i][j].sel3)))
                continue;
            if (!inst[i][j].sel4 && inst[i][j].op == 1) //是sch1除法的最后一个周期
            {
                if (!inst[i][j].sel2) //冲突情况 因为此拍rt无输入
                {
                    float A = Ax[inst[i][j].rd];
                    ...
                    std::cout << A << " / " << B << " = " << Ax[inst[i][j].rd] << std::endl;
                }
                else //未冲突情况
                {
                    float A = Ax[inst[i][j].rd];
            ...
            else if (inst[i][j].op == 0)//是ms的最后一个周期
            {
                float A, B, C;
                if (inst[i][j].sel1 && inst[i][j].sel2) //情况1 111
                {
                    A = Ax[inst[i][j].rs];
                    B = Ax[inst[i][j].rt];
                }
                else if (inst[i][j].sel1 && !inst[i][j].sel2) //情况2 101
                {
                    A = Ax[inst[i][j].rs];
                    B = Ax[inst[i - 1][j].rt];
                }
                else if (inst[i - 1][j].sel2) //情况34  前一拍的sel2 == 1
                {
                    A = Ax[inst[i - 1][j].rs];
                    B = Ax[inst[i - 1][j].rt];
                }
            ...
            //sch2中增加的内容
            //减法，检测到200必执行
            else if (inst[i][j].op == 2 && inst[i][j].sel1 == 0 && inst[i][j].sel2 == 0)
            {
                float X;
                float Y;
                Y = regC[j];
            ...
            //  reg写回操作 3000
            else if (inst[i][j].op == 3 && inst[i][j].sel3 == 0)
            {
                Ax[inst[i][j].rd] = reg[j];
                std::cout << "time: " << i << "\tPE: " << j << "\top: ";
                std::cout << "Ax[" << inst[i][j].rd << "] = " << Ax[inst[i][j].rd] << endl; 
            }
            // 只有一个除法时写入reg的操作 3001
            else if (inst[i][j].op == 3 && inst[i][j].sel3 == 1)
            {
                reg[j] = Ax[inst[i][j].rd];
                std::cout << "time: " << i << "\tPE: " << j << "\top: ";
                std::cout << "reg[" << j << "] = " << reg[j] << endl; 
            }
            // sch2的除法操作
            else if (inst[i][j].sel4 && inst[i][j].op == 1)
            {
                float B = Ax[inst[i][j].rt];
                float A = reg[j];
                if (B == 0)
                ...
                reg[j] = A / B;
                std::cout << "time: " << i << "\tPE: " << j << "\top: ";
                std::cout << A << " / " << B << " = " << reg[j] << endl; 
            }
            // 乘法累加操作,AB不冲突
            else if (inst[i][j].op == 2 && inst[i][j].sel4 && inst[i][j].sel2)
            {
                float C = regC[j];
    ...
    for (int i = 0; i < time; i++) {
        for (int j = 0; j < NPE; j++) {
            if (inst[i][j].rs == -1)
                ...
                bs = intToBinaryString16Bit(inst[i][j].rs);
                file << bs;
            }
            file << "_";
            if (inst[i][j].rt == -1)
                ...
                bs = intToBinaryString16Bit(inst[i][j].rt);
                file << bs;
            }
            file << "_";
            if (inst[i][j].rd == -1)
                ...
                bs = intToBinaryString16Bit(inst[i][j].rd);
                file << bs;
            }
            file << "_";
            if (inst[i][j].valid) {
            ...
            file << inst[i][j].sel1 << "_";
            file << inst[i][j].sel2 << "_";
            file << inst[i][j].sel3 << "_";
            file << inst[i][j].valid << std::endl;
        }
    }
